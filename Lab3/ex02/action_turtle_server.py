...
class CommandsActionServer(Node):
    ...
    def execute_callback(self, goal_handle):
        ...
        if goal_handle.request.command == 'forward':
            ...
            while int(feedback_msg.odom) != goal_handle.request.s:
                feedback_msg.odom = math.sqrt((position.x - x0)**2+(position.y - y0)**2)
                self.get_logger().info(f'I went: {position.x} {x0} m')
                self.get_logger().info(f'ODOM:{feedback_msg.odom}, CLEAR ODOM:{math.sqrt((position.x - x0)**2+(position.y - y0)**2)}')
